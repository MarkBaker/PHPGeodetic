...
class RotationMatrixValuesTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $this->_xDistance = $this->getMock('Geodetic_Angle');
        $this->_xDistance->expects($this->any())
            ->method('getValue')
            ->will($this->returnValue(15.0));
        $this->_yDistance = $this->getMock('Geodetic_Angle');
        $this->_yDistance->expects($this->any())
            ->method('getValue')
            ->will($this->returnValue(30.0));
        $this->_zDistance = $this->getMock('Geodetic_Angle');
    ...
    {
        $matrixObject = new Geodetic_RotationMatrixValues();
        //    Must return an object...
        $this->assertTrue(is_object($matrixObject));
        //    ... of the correct type
        $this->assertTrue(is_a($matrixObject, 'Geodetic_RotationMatrixValues'));
